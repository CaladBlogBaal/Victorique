...
class Games(commands.Cog):
    ...
    @commands.command()
    @checking_for_multiple_channel_instances()
    async def hangman(self, ctx, word_length: int = 0):
        """
        start a game of hangman
        """
        # this will be reaction based soon

        self.multiple_user_instance_set(ctx)

        def _embed(wt, correct_guesses_, incorrect_guesses_):

            correct_guesses_ = set(correct_guesses_)
            incorrect_guesses_ = set(incorrect_guesses_)
            incorrect_guesses_ = ", ".join(incorrect_guesses_)
            correct_guesses_ = ", ".join(correct_guesses_)

            embed = discord.Embed(title=f"Hangman game",
                                  description=f"\nWord to be guessed of length {len(wt)}"
                                  f"\nWord: {wt}"
                                  f"\nCorrect letters: {correct_guesses_}"
                                  f"\nIncorrect letters: {incorrect_guesses_}",
                                  color=self.bot.default_colors())

            embed.set_footer(text=f'Requested by {ctx.message.author.name} powered by'
                                  f' https://github.com/dwyl/english-words', icon_url=ctx.message.author.avatar_url)
            embed.set_image(url=hangman_images[index])
            embed.timestamp = ctx.message.created_at
        ...
        guesses = 7
        hangman_images = [None,
                          "https://i.imgur.com/pWaenqk.png",
                          "https://i.imgur.com/umcLb12.png",
                          "https://i.imgur.com/jQe8ar0.png",
                          "https://i.imgur.com/WgZPoME.png",
                          "https://i.imgur.com/t6usQUj.png",
                          "https://i.imgur.com/5tKxW1j.png",
                          "https://i.imgur.com/5tKxW1j.png",
                          "https://i.imgur.com/8rSim4U.png"]
        ...
        if word_length < 3 and word_length != 0:
            word_length = 3

        if word_length == 0:
            word = random.choice(open(r"config/words_alpha.txt").read().split())
        ...
        msg = await ctx.send(embed=_embed(word_teaser, correct_guesses, incorrect_guesses))

        word_teaser_list = list(x for x in word_teaser)

        while True:

            m = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=60)

            while len(m.content) != 1:
                m = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=60)

            while len(m.content) == 1 and m.content.isalpha() is False or len(re.findall(r"<:\w*:\d*>", m.content)) != 0:
                m = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=60)

            if m.content in word:
                correct_guesses.append(m.content)
                i = [i for i, letter in enumerate(word) if letter == m.content]
                for value in i:
                    word_teaser_list[value] = m.content

                word_teaser_new = "".join(e for e in word_teaser_list)

                await ctx.send(f"you guessed a correct letter {ctx.author.name}({m.content}) "
                               f"number of guesses left {guesses}", delete_after=1)

                await msg.edit(embed=_embed(word_teaser_new, correct_guesses, incorrect_guesses))

                # checking if they guessed the correct word
                if word_teaser_new == word:
                    return await ctx.send(f"Successfully guessed the correct word {word}, time taken "
                                          f"{round(time.clock() - start, 2)} seconds Attempts taken "
                                          f"{(7 - guesses) + len(word)}")
            else:

                incorrect_guesses.append(m.content)
                guesses -= 1
                await ctx.send(f"{m.content} is not in this word, guesses deducted by one, number of guesses left "
                               f"{guesses}", delete_after=3)

                word_teaser_new = "".join(e for e in word_teaser_list)
                await msg.edit(embed=_embed(word_teaser_new, correct_guesses, incorrect_guesses))

            if guesses == 0:
                return await ctx.send(f"> failed to successfully guess the word {word}")
